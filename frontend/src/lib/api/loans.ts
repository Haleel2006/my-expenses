...
export const deleteLoan = async (userId: string, loan: Loan) => {
  if (!loan.id) return;
  const batch = writeBatch(db);
  ...
  // 2. Update Balances ONLY if the loan was pending
  if (loan.status === 'pending') {
    ...
    if (balanceSnap.exists()) {
      ...
      if (loan.type === 'given') {
        loansReceivable = Math.max(0, loansReceivable - loan.amount);
      } else {
        loansPayable = Math.max(0, loansPayable - loan.amount);
      }
