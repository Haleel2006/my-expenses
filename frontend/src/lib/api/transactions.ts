...
import { collection, doc, getDoc, getDocs, query, orderBy, writeBatch, Timestamp, increment, limit } from 'firebase/firestore';
...
export const fetchBalances = async (userId: string): Promise<Balances | null> => {
...
export const addTransaction = async (userId: string, transaction: Transaction) => {
  ...
  const txData: Omit<Transaction, 'date' | 'createdAt'> & { date: Timestamp; createdAt: Timestamp } = {
...
export const deleteTransaction = async (userId: string, transactionId: string, transaction: Transaction) => {
  ...
  if (balanceSnap.exists()) {
    ...
    const isWallet = transaction.paymentMethod === 'Wallet' || (transaction.paymentMethod as string) === 'Cash';
