...
export default function GoalsPage() {
  ...
  const loadGoals = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Error fetching goals", description: err.message, variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }, [user, toast]);
  ...
  const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string; fill?: string; payload: unknown }[]; label?: string }) => {
